        ...
        switch (rules[i].token_type) {
          ...
          case(TK_MINUS):
            if (is_binary_operator){
              ...
              strncpy(tokens[nr_token].str,"Neg",4);
            }
            nr_token++;
            break;
    ...
    switch(typeof_token(&(expr.tokens[i]))){
      ...
      case(TK_TYPE_VALUE):
        if(expr.tokens[i].type==TK_REG){
          expr.tokens[i].num_value=isa_reg_str2val(expr.tokens[i].str+1,success);
          printf("Accessing %s, value %ld.\n",expr.tokens[i].str,expr.tokens[i].num_value);
          if(!*success){
